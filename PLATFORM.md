...
**`rbx-harness`** é a infraestrutura de runtime de agentes da plataforma RBX.

Este repositório implementa o ambiente de execução sobre o qual agentes operam: isolamento de sessão, gestão de recursos, ciclo de vida de processos e instrumentação de execução. rbx-harness é a camada de infraestrutura do Agent Orchestration Plane — enquanto rbx-maestro coordena o que deve ser executado, o harness garante que a execução aconteça de forma isolada, monitorável e segura.

rbx-harness não avalia qualidade de agentes, não roteia tráfego AI e não registra decisões institucionais. Ele provê: o ambiente de execução confiável que torna possível executar agentes com rastreabilidade e contenção de falhas.
